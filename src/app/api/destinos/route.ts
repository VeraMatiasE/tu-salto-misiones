...
export async function GET(request: NextRequest) {
  try {
    ...
    const ubicaciones = searchParams.get('ubicaciones')?.split(',').filter(Boolean) || []
    const dificultades = searchParams.get('dificultades')?.split(',').filter(Boolean) || []
    ...
    const puntuacionMin = puntuacionMinStr ? parseFloat(puntuacionMinStr) : undefined
    const puntuacionMax = puntuacionMaxStr ? parseFloat(puntuacionMaxStr) : undefined
    const page = pageStr ? parseInt(pageStr, 10) : 1
    const limit = limitStr ? parseInt(limitStr, 10) : 100

    if (puntuacionMinStr && isNaN(puntuacionMin as number)) {
      return NextResponse.json({ error: 'puntuacionMin debe ser un número válido' }, { status: 400 })
    }
    if (puntuacionMaxStr && isNaN(puntuacionMax as number)) {
      return NextResponse.json({ error: 'puntuacionMax debe ser un número válido' }, { status: 400 })
    }
    if (pageStr && (isNaN(page) || page < 1)) {
      return NextResponse.json({ error: 'page debe ser un número entero mayor a 0' }, { status: 400 })
    }
    if (limitStr && (isNaN(limit) || limit < 1)) {
      return NextResponse.json({ error: 'limit debe ser un número entero mayor a 0' }, { status: 400 })
    }
