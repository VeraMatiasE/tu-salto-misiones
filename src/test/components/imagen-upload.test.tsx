...
jest.mock('next-cloudinary', () => ({
  ...
  CldImage: ({ src, alt, fill, onLoad, onError, ...props }: { 
    src: string; 
    alt: string; 
    fill: boolean; 
    onLoad?: () => void;
...
describe('ImagenUpload', () => {
  ...
  describe('Renderizado inicial', () => {
    it('debe renderizar las pestañas correctamente', () => {
      ...
      expect(screen.getByRole('tab', { name: 'Subir imágenes' })).toBeInTheDocument()
    })

    it('debe mostrar la pestaña de galería por defecto', () => {
      render(<ImagenUpload {...defaultProps} />)
      
      expect(screen.getByRole('tab', { name: 'Galería' })).toHaveAttribute('data-state', 'active')
      expect(screen.getByRole('tab', { name: 'Subir imágenes' })).toHaveAttribute('data-state', 'inactive')
    })

    it('debe mostrar el botón de volver a la galería', () => {
      render(<ImagenUpload {...defaultProps} />)
      
      expect(screen.getByRole('button', { name: 'Volver a la galería' })).toBeInTheDocument()
    })
  })
  ...
  describe('Pestaña de Galería - Sin imágenes', () => {
    it('debe mostrar estado vacío cuando no hay imágenes', () => {
      ...
      expect(screen.getByText('Este destino aún no tiene imágenes.')).toBeInTheDocument()
      expect(screen.getByRole('button', { name: 'Subir imágenes' })).toBeInTheDocument()
    })
    ...
    it('debe cambiar a pestaña de subir al hacer clic en el botón', async () => {
      ...
      const uploadButton = screen.getByRole('button', { name: 'Subir imágenes' })
      await user.click(uploadButton)
      
      expect(screen.getByRole('tab', { name: 'Subir imágenes' })).toHaveAttribute('data-state', 'active')
    })
  })
  ...
  describe('Pestaña de Subir imágenes', () => {
    beforeEach(async () => {
      ...
      const uploadTab = screen.getByRole('tab', { name: 'Subir imágenes' })
    ...
    it('debe mostrar las recomendaciones para imágenes', () => {
      expect(screen.getByText('Recomendaciones para imágenes')).toBeInTheDocument()
      expect(screen.getByText(/usa imágenes de alta calidad/i)).toBeInTheDocument()
    })
  ...
  describe('Selección de archivos', () => {
    ...
    it('debe permitir seleccionar archivos válidos', async () => {
      ...
      expect(screen.getByRole('button', { name: 'Subir (1)' })).not.toBeDisabled()
    })
    ...
    it('debe permitir seleccionar múltiples archivos', async () => {
      ...
      expect(screen.getByLabelText('Vista previa')).toHaveTextContent('Vista previa (2 imagenes)')
      expect(screen.getByRole('button', { name: 'Subir (2)' })).not.toBeDisabled()
    })
  })
  ...
  describe('Vista previa de archivos', () => {
    ...
    it('debe mostrar vista previa de archivos seleccionados', () => {
      ...
      expect(screen.getByTestId('next-image')).toHaveAttribute('src', 'mock-blob-url')
    })
  ...
  describe('Subida de archivos', () => {
    ...
    it('debe manejar errores durante la subida', async () => {
      ...
      const uploadButton = screen.getByRole('button', { name: 'Subir (1)' })
      await user.click(uploadButton)
      
      await waitFor(() => {
        expect(screen.getByText(/ocurrió un error al subir/i)).toBeInTheDocument()
      })
  ...
  describe('Eliminación de imágenes', () => {
    ...
    it('debe mostrar diálogo de confirmación al eliminar', async () => {
      ...
      expect(screen.getByLabelText('¿Eliminar esta imagen?')).toHaveTextContent('Esta acción no se puede deshacer')
    })
  ...
  describe('Navegación', () => {
    it('debe navegar de vuelta a la galería principal', async () => {
      ...
      const backButton = screen.getByRole('button', { name: 'Volver a la galería' })
  ...
  describe('Casos edge', () => {
    it('debe manejar imágenes sin fecha de actualización', () => {
      ...
      render(<ImagenUpload saltoId="test-salto" initialImages={imagesWithoutDate} />)
