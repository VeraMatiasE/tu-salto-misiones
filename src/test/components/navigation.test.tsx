...
jest.mock('next-cloudinary', () => ({
  ...
  CldImage: ({ src, alt, ...props }: MockCldImageProps) => <img src={src} alt={alt} {...props} />,
}));
...
describe('Navigation Component', () => {
  const mockUseMobileMenu = jest.requireMock('@/hooks/use-mobile-menu').useMobileMenu as jest.Mock<MockUseMobileMenuReturn, []>;
  ...
  describe('Variante default - Usuario autenticado', () => {
    const mockUserProfile: MockUserProfile = {
      user: { id: '1', email: 'test@example.com', nombre: 'Usuario Test' },
      profile: { 
        id: '1', 
        nombre: 'Usuario Test', 
        foto_perfil: null,
    ...
    test('debe renderizar imagen de perfil cuando existe', async () => {
      const userWithPhoto: MockUserProfile = {
        ...mockUserProfile,
        profile: { ...mockUserProfile.profile, nombre: 'Usuario Mock', foto_perfil: 'test-image-url' }
      };
