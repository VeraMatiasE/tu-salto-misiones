...
describe('SaltosList', () => {
  ...
  const createMockResponse = (data: ApiResponse, options: Partial<MockResponse> = {}): MockResponse => ({
    ok: true,
    status: 200,
    statusText: 'OK',
  ...
  describe('Renderizado básico', () => {
    ...
    test('muestra mensaje cuando no hay saltos', () => {
      ...
      expect(screen.getByText('No hay saltos registrados. Agrega uno nuevo para comenzar.')).toBeInTheDocument()
    })
  })
  ...
  describe('Renderizado de datos', () => {
    ...
    test('renderiza links de ubicación correctamente', () => {
      ...
      const linkMocona = screen.getByRole('link', { name: /El Soberbio, Misiones/i })
      expect(linkMocona).toHaveAttribute('href', 'https://maps.google.com/mocona')
  ...
  describe('Botones de acción', () => {
    ...
    test('links de edición apuntan a la URL correcta', () => {
      ...
      expect(editLinksFiltered[0]).toHaveAttribute('href', '/dashboard/saltos/1')
      expect(editLinksFiltered[1]).toHaveAttribute('href', '/dashboard/saltos/2')
      expect(editLinksFiltered[2]).toHaveAttribute('href', '/dashboard/saltos/3')
    })
  })
  ...
  describe('Funcionalidad de eliminación', () => {
    test('abre dialog de confirmación al hacer clic en eliminar', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText(/Se eliminará permanentemente el registro de Salto del Moconá/)).toBeInTheDocument()
      })
    })
    ...
    test('elimina salto exitosamente con callback', async () => {
      ...
      render(<SaltosList saltos={[mockSaltos[0]]} onSaltoDeleted={mockOnSaltoDeleted} />)
    ...
    test('maneja errores de red correctamente', async () => {
      ...
      await waitFor(() => {
        expect(alertSpy).toHaveBeenCalledWith('Error al eliminar Salto del Moconá: Network error')
      })
    ...
    test('maneja errores HTTP correctamente', async () => {
      ...
      await waitFor(() => {
        expect(alertSpy).toHaveBeenCalledWith('Error al eliminar Salto del Moconá: Salto no encontrado')
      })
    ...
    test('maneja errores HTTP sin mensaje personalizado', async () => {
      ...
      const errorResponse: MockResponse = {
        ...
        json: jest.fn<Promise<ApiResponse>, []>().mockRejectedValue(new Error('Invalid JSON'))
      }
      mockFetch.mockResolvedValueOnce(errorResponse as unknown as Response)
      ...
      await waitFor(() => {
        expect(alertSpy).toHaveBeenCalledWith('Error al eliminar Salto del Moconá: Error 500: Internal Server Error')
      })
